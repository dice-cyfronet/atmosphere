...
describe HttpMappingProxyConfGenerator do
  ...
  describe "#run" do
    ...
    context 'when production appliance started on cloud site' do
      ...
      context 'with http port mapping' do
      ...
      context 'with https port mapping' do
      ...
      context 'with http_https port mapping' do
  ...
  def redirection(appl, pmt, vms, type)
    {
      path: "#{appl.appliance_set.id}/#{appl.appliance_configuration_instance.id}/#{pmt.service_name}",
