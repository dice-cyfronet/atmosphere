...
class BillingCharger
  ...
  def self.bill_all_appliances
    Appliance.all.each do |appl|
      Rails.logger.debug("Billing appliance #{appl.id}...")
      ...
      billing_interval = (current_time - appl.prepaid_until) # This will return time in seconds
      ...
      if billing_interval < 0
        ...
        ActiveRecord::Base.transaction do
          ...
          if amount_due > appl.fund.balance-appl.fund.overdraft_limit
            ...
            Rails.logger.warn("The balance of fund #{fund.id} is insufficient to cover continued operation of appliance #{appl.id}. Flagging appliance as expired.")
            ...
            appl.last_billing = current_time
            appl.billing_state=:prepaid
            unless appl.save
              Rails.logger.error("ERROR: Unable to update appliance #{appl.id} with billing data.")
            else
              # Write to log.
              log = BillingLog.new(username: appl.appliance_set.user.login, appliance: appl.id.to_s+'---'+(appl.name.blank? ? 'unnamed_appliance' : appl.name), fund: appl.fund.name, actor: "bill_all_appliances", amount_billed: amount_due)
            end
          end
        end
      end
  ...
  def self.calculate_charge_for_appliance(appl, billing_time)
    ...
    billable_time = ((billing_time - appl.prepaid_until)/@appliance_prepayment_interval) + 1.0 # Time in hours
    ...
    appl.virtual_machines.each do |vm|
      ...
      Rails.logger.debug("Total estimated cost of using VM #{vm.id} over a period of #{billable_time} hours is #{charge}")
      amount_due += charge
    end

    Rails.logger.debug("Total estimated charge for using Appliance #{appl.id} over a period of #{billable_time} hours is #{amount_due}")
