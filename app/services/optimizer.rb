...
class Optimizer
  ...
  def select_tmpl_and_flavor(tmpls, options={})
    ...
    tmpls.each do |tmpl|
      opt_fl = (min_elements_by(tmpl.compute_site.virtual_machine_flavors.active.select {|f| (f.supported_architectures == 'i386_and_x86_64' || f.supported_architectures == tmpl.architecture) && f.memory >= required_mem && f.cpu >= required_cores && f.hdd >= required_disk}) {|f| f.hourly_cost}).sort!{ |x,y| y.memory <=> x.memory }.last
